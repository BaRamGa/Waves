...
class MatcherSnapshotStore(config: Config) extends SnapshotStore with ActorLogging {
  ...
  private var pendingActions = Map.empty[String, Future[Unit]]
    ...
    Future(readOnly { db =>
      db.get(kSMHistory(persistenceId))
        ...
        .map { seqNr =>
          ...
          SelectedSnapshot(SnapshotMetadata(persistenceId, sm.seqNr, sm.ts), deserialize(db.get(kSnapshot(persistenceId, seqNr))))
        }
    })(streamDispatcher)

  private def nextAction(message: String, persistenceId: String, f: => Unit): Future[Unit] = {
    log.info(message)
    val newAction = pendingActions
      .getOrElse(persistenceId, Future.unit)
      .transform(_ => Try(f))(streamDispatcher)
    pendingActions += persistenceId -> newAction
    newAction
  }

  override def saveAsync(metadata: SnapshotMetadata, snapshot: Any): Future[Unit] =
    nextAction(s"About to save snapshot for ${metadata.persistenceId}", metadata.persistenceId, save(metadata, snapshot))

  override def deleteAsync(metadata: SnapshotMetadata): Future[Unit] =
    nextAction(
      s"About to delete snapshot for ${metadata.persistenceId}",
  ...
  override def deleteAsync(persistenceId: String, criteria: SnapshotSelectionCriteria): Future[Unit] =
    nextAction(
      s"About to delete a series of snapshots for $persistenceId",
      persistenceId,
      readWrite { db =>
  ...
  private def migrate(): Unit = {
    log.info(s"Opening db at ${snapshotDir()}")
    ...
    if (allSnapshots.nonEmpty) {
      ...
      for (sm <- allSnapshots) {
        Try(withInputStream(sm)(is => deserialize(streamToBytes(is)))) match {
          case Success(s) => save(sm, s.data)
          case Failure(e) => log.error(e, s"Error reading snapshot $sm")
        }
      }
    } else {
...
object MatcherSnapshotStore {
  case class SM(seqNr: Long, ts: Long) {
    ...
    def matches(metadata: SnapshotMetadata): Boolean =
      seqNr == metadata.sequenceNr && ts == metadata.timestamp
  }
  ...
  private def kSnapshot(persistenceId: String, seqNr: Int) =
    Key[Array[Byte]](Bytes.concat(Array(2: Byte), persistenceId.getBytes(UTF_8), Ints.toByteArray(seqNr)), identity, identity)
}
